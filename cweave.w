...
\def\title{CWEAVE-in-Go (Version 0.1)}
\def\topofcontents{\null\vfill
	\centerline{\titlefont The {\ttitlefont CWEAVE-in-Go} processor}
	\vskip 15pt
...
@** Introduction.
This is the \.{CWEAVE-in-Go} program by Alexander Syhcev
based on \.{CWEAVE-in-Go} by Silvio Levy and Donald E. Knuth,
...
The ``banner line'' defined here should be changed whenever \.{CWEAVE-in-Go}
is modified.
...
@ \.{CWEAVE-in-Go} has a fairly straightforward outline.  It operates in
...
to \.{CTANGLE} and \.{CWEAVE-in-Go}, before proceeding further.
...
handle \TEX/, so they should be sufficient for most applications of \.{CWEAVE-in-Go}.
...
max_scraps = 2000 /* number of tokens in \CEE/ texts being parsed */
stack_size = 400 /* number of simultaneous output levels */

@ The next few sections contain stuff from the file |"common.w"| that must
be included in both |"ctangle.w"| and |"cweave.w"|. 

@i common.w

@* Data structures exclusive to {\tt CWEAVE-in-Go}.
...
@<More elements of |name_info| structure@>=
	ilk		int32 /* used by identifiers in \.{CWEAVE-in-Go} only */
...
@ The other large memory area in \.{CWEAVE-in-Go} keeps the cross-reference data.
...
the length of the list, under foreseeable uses of \.{CWEAVE-in-Go} this inefficiency
is insignificant.
...
to insert them every time \.{CWEAVE-in-Go} is run.  Fortunately there are relatively
...
into a numeric code designed to simplify \.{CWEAVE-in-Go}'s logic; for example,
...
@ @<Constants@>=
ignore rune = 00 /* control code of no interest to \.{CWEAVE-in-Go} */
verbatim rune = 02 /* takes the place of extended ASCII \.{\char2} */
begin_short_comment rune = 03 /* \CPLUSPLUS/ short comment */
...
@ Control codes are converted to \.{CWEAVE-in-Go}'s internal
...
As stated above, \.{CWEAVE-in-Go}'s most interesting lexical scanning routine is the
...
@ @<Constants@>=
constant = 0200 /* \CEE/ constant */
str = 0201 /* \CEE/ string */
identifier = 0202 /* \CEE/ identifier or reserved word */
...
introduced by \.0 and hexadecimals by \.{0x}, but \.{CWEAVE-in-Go} will print
...
\.{CWEAVE-in-Go}'s first pass over the source file. If everything works right,
both phase one and phase two of \.{CWEAVE-in-Go} will assign the same numbers to
...
func copy_comment(
	is_long_comment bool, /* is this a traditional \CEE/ comment? */
...
@** Parsing.
The most intricate part of \.{CWEAVE-in-Go} is its mechanism for converting
...
\CEE/-like material, since \.{CWEAVE-in-Go} must deal with fragmentary
...
@ This code allows \.{CWEAVE-in-Go} to display its parsing steps.
...
interpreted by \.{CWEAVE-in-Go} before they are written to the output file.
...
@ The production rules listed above are embedded directly into \.{CWEAVE-in-Go},
...
@ If \.{CWEAVE-in-Go} is being run in debugging mode, the production numbers and
...
@ Some nonstandard characters may have entered \.{CWEAVE-in-Go} by means of
standard ones. They are converted to \TEX/ control sequences so that it is
possible to keep \.{CWEAVE-in-Go} from outputting unusual |rune| codes.
...
\.{CWEAVE-in-Go}'s internal memory; we have to figure out how to get them into
...
this part of \.{CWEAVE-in-Go} was designed.  (a) There are two modes of output:
...
@ @<Constants@>=
inner mode = 0 /* value of |mode| for \CEE/ texts within \TEX/ texts */
outer mode = 1 /* value of |mode| for \CEE/ texts in sections */
...
func output_C() {
	...
	next_control=ignore
	p:=C_translate()/* translation of the \CEE/ text */
...
@ Here is \.{CWEAVE-in-Go}'s major output handler.
...
@ The current mode does not affect the behavior of \.{CWEAVE-in-Go}'s output routine
...
the processing in \.{CWEAVE-in-Go}'s main pass over the source file. Phase two
...
@ @c
/* finishes a definition or a \CEE/ part */
...
Note that the output of \.{CWEAVE-in-Go} is not English-specific; users may
...
We are nearly finished! \.{CWEAVE-in-Go}'s only remaining task is to write out the
...
\.{CWEAVE-in-Go}'s parsing algorithm and rename them |depth| and |head|. They now
...
@ \.{CWEAVE-in-Go} specific creation of output file
