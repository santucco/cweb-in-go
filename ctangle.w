...
\def\title{CTANGLE-in-Go (Version 0.1)}
\def\topofcontents{\null\vfill
  \centerline{\titlefont The {\ttitlefont CTANGLE-in-Go} processor}
  \vskip 15pt
...
@** Introduction.
This is the \.{CTANGLE-in-Go} program by Alexander Sychev,
based on \.{CTANGLE-in-Go} by Silvio Levy and Donald E. Knuth.

The ``banner line'' defined here should be changed whenever \.{CTANGLE-in-Go}
is modified.
...
@ \.{CTANGLE-in-Go} has a fairly straightforward outline.  It operates in
...
to \.{CTANGLE-in-Go} and \.{CWEAVE-in-Go}, before proceeding further.
...
@* Data structures exclusive to {\tt CTANGLE-in-Go}.
...
\.{CWEAVE-in-Go}, but we need to declare a dummy version so that
...
@ \{CTANGLE-in-Go} operates with UTF8 encoding texts and represents a text in
...
space or line breaks allowed between them (the \.{@@\&} operation of \.{CWEB-in-Go}).
...
effect on \.{CTANGLE-in-Go}'s algorithms.
...
compiler won't complain.  By default, \.{CTANGLE-in-Go} converts the
...
We have now seen that \.{CTANGLE-in-Go} will be able to output the full
...
\TEX/ text in the \.{CWEB-in-Go} source code. One of the latter routines will jump to
the next token that starts with `\.{@@}', and the other skips to the end
of a \CEE/ comment.

@ Control codes in \.{CWEB-in-Go} begin with `\.{@@}', and the next character
identifies the code. Some of these are of interest only to \.{CWEAVE-in-Go},
so \.{CTANGLE-in-Go} ignores them; the others are converted by \.{CTANGLE-in-Go} into
...
@<Constants@>=
ignore rune = 0 /* control code of no interest to \.{CTANGLE-in-Go} */
...
speed in order to pass over comments, which \.{CTANGLE-in-Go} does not transmit
...
@ \CEE/ strs and character constants, delimited by double and single
...
\.{CWEAVE-in-Go} and \.{CTANGLE-in-Go} will both assign the same numbers to sections.


@ The body of |scan_section| is a loop where we look for control codes
that are significant to \.{CTANGLE-in-Go}: those
...
\.{CTANGLE-in-Go} does not enforce this rule; it simply ignores the offending
...
@ \.{CTANGLE-in-Go} specific creation of output file
...
@** Index.
Here is a cross-reference table for \.{CTANGLE-in-Go}.
