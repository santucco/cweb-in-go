...
\def\title{Common code for CTANGLE-in-Go and CWEAVE-in-Go (Version 0.1)}
\def\topofcontents{\null\vfill
  \centerline{\titlefont Common code for {\ttitlefont CTANGLE-in-Go} and
    {\ttitlefont CWEAVE-in-Go}}
  \vskip 15pt
...
@** Introduction.  This file contains code common to both
\.{CTANGLE-in-Go} and \.{CWEAVE-in-Go}, which roughly concerns
the following problems: character uniformity, input routines,
error handling and parsing of command line. 
...
@h
@<Definitions that should agree with \.{CTANGLE-in-Go} and \.{CWEAVE-in-Go}@>@/
@<Other definitions@>@/

@ \.{CWEAVE-in-Go} operates in three phases: First it inputs the source
...
Similarly, \.{CTANGLE-in-Go} operates in two phases.
The global variable |phase| tells which phase we are in.

@<Other...@>= var phase int /* which phase are we in? */

@ There's an initialization procedure that gets both \.{CTANGLE-in-Go} and
\.{CWEAVE-in-Go} off to a good start. We will fill in the details of this
procedure later.

@c
func common_init() {
  @<Initialize pointers@>
  @<Set the default options common to \.{CTANGLE-in-Go} and \.{CWEAVE-in-Go}@>
...
substituted. The indexes to \.{CTANGLE-in-Go} and \.{CWEAVE-in-Go} mention every
...
@ @<Common constants@>=
and_and = 04 /* `\.{\&\&}'\,; corresponds to MIT's {\tentex\char'4} */
lt_lt = 020 /* `\.{<<}'\,;  corresponds to MIT's {\tentex\char'20} */
...
@** Input routines.  The lowest level of input to the \.{CWEB-in-Go} programs
...
Recall that the actual text that \.{CWEB-in-Go} should process comes from two
...
for the benefit of \.{CTANGLE-in-Go}.
...
@ The |reset_input| procedure, which gets \.{CWEB-in-Go} ready to read the
user's \.{CWEB-in-Go} input, is used at the beginning of phase one of \.{CTANGLE-in-Go},
phases one and two of \.{CWEAVE-in-Go}.
...
the |file[include_depth]| has changed, we tell \.{CTANGLE-in-Go} to print this
...
var print_where bool = false /* should \.{CTANGLE-in-Go} print line and file info? */
...
double quotes.
If the environment variable \.{CWEBINPUTS} is set
\.{CWEB-in-Go} will look for include files in the directory thus named, if
it cannot find them in the current directory.
(Colon-separated paths are supported.)
...
Both \.{CWEAVE-in-Go} and \.{CTANGLE-in-Go} store identifiers, section names and
...
\.{CWEAVE-in-Go} and \.{CTANGLE-in-Go}.  If there is no match for the identifier,
...
func id_lookup(
	...
	t int32 /* the |ilk|; used by \.{CWEAVE-in-Go} only */) int32 { 
...
in a slightly different way in \.{CWEAVE-in-Go} than in \.{CTANGLE-in-Go}; hence the
|init_p| procedure.
...
\.{CTANGLE-in-Go} needs to be able to recognize a section name when given a prefix of
...
differently in \.{CWEAVE-in-Go} and \.{CTANGLE-in-Go}; hence the
|init_node| procedure, which is defined differently in \.{CWEAVE-in-Go.w}
and \.{ctangle.w}.
...
\.{CTANGLE-in-Go} and \.{CWEAVE-in-Go} have their own notions about how to
...
@ An overflow stop occurs if \.{CWEB-in-Go}'s tables aren't large enough.
...
and \.{CWEB-in-Go} prints an error message that is really for the \.{CWEB-in-Go}
...
The user calls \.{CWEAVE-in-Go} and \.{CTANGLE-in-Go} with arguments on the command line.
...
@ @<Defin...@>=
var c_file_name string /* name of |c_file| */
...
accordingly.  At least one file name must be present: the \.{CWEB-in-Go}
file.  It may have an extension, or it may omit the extension to get |".w"| or
|".web"| added.  The \TEX/ output file name is formed by replacing the \.{CWEB-in-Go}
...
func scan_args() {
	...
	for i := 1; i < len(os.Args); i++  {
		...
		if (arg[0]=='-' || arg[0]=='+') && len(arg) > 1 {
			...
			if !found_web {
				@<Make |file_name[0]|, |tex_file_name|, and |c_file_name|@>
			} else if !found_change {
				@<Make |change_file_name| from |fname|@>
			} else if !found_out {
				@<Override |tex_file_name| and |c_file_name|@>
			} else {
...
@<Defin...@>=
var c_file io.WriteCloser /* where output of \.{CTANGLE-in-Go} goes */
var tex_file io.WriteCloser /* where output of \.{CWEAVE-in-Go} goes */
var idx_file io.WriteCloser /* where index from \.{CWEAVE-in-Go} goes */
var scn_file io.WriteCloser /* where list of sections from \.{CWEAVE-in-Go} goes */
var active_file io.WriteCloser /* currently active file for \.{CWEAVE-in-Go} output */
